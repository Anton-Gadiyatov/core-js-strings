...
function formatTime(minutes, seconds) {
  const minutesWithPad = `${minutes}`.padStart(2, '0');
  const secondWithPad = `${seconds}`.padStart(2, '0');
  return `${minutesWithPad}:${secondWithPad}`;
}
...
function orderAlphabetically(str) {
  let initialStr = str;
  let sortedStr = '';
  while (initialStr.length > 0) {
    let minChar = initialStr[0];
    for (let i = 0; i < initialStr.length; i += 1) {
      if (initialStr[i] < minChar) {
        minChar = initialStr[i];
      }
    }
    sortedStr += minChar;
    initialStr = initialStr.replace(minChar, '');
  }
  return sortedStr;
}
...
function isPalindrome(str) {
  ...
  let formattedStr = '';
  for (let i = 0; i < str.length; i += 1) {
    if (letters.includes(str[i].toLocaleLowerCase())) {
      formattedStr += str[i].toLocaleLowerCase();
    }
  }
...
function invertCase(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (str[i] === str[i].toUpperCase()) {
      inversedCaseStr += str[i].toLowerCase();
    } else {
      inversedCaseStr += str[i].toUpperCase();
    }
  }
...
function getCardId(value) {
  ...
  const shirts = '♣♦♥♠';
  const shirtIndex = shirts.indexOf(value.slice(-1));
  const cardIndex = cards.split(',').indexOf(value.slice(0, value.length - 1));
  return shirtIndex
    ? +`${shirtIndex * cards.split(',').length + cardIndex}`
    : +cardIndex;
}
